"""OPTIMIZED | Base classes for objects that can be displayed."""
...
class Mobject:
    ...
    def get_merged_array(self, array_attr: str) -> np.ndarray:
        ...
        def collect(mob):
            arrays.append(getattr(mob, array_attr))
            for submob in mob.submobjects:
                collect(submob)

        collect(self)

        return np.concatenate(arrays, axis=0) if arrays else np.empty((0, self.dim))
    ...
    def get_family(self, recurse: bool = True) -> list[Mobject]:
        ...
        while stack:
            mob = stack.pop()
            result.append(mob)
            stack.extend(reversed(mob.submobjects))
        return remove_list_redundancies(result)
