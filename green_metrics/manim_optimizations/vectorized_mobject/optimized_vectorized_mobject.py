...
class VMobject(Mobject):
    ...
    def generate_rgbas_array(
        ...
        if sheen_factor != 0 and len(rgbas) == 1:
            rgbas = np.tile(rgbas, (2, 1))

            # Apply the sheen to only the bottom half of the array block
            half_idx = len(rgbas) // 2
            rgbas[half_idx:, :3] = np.clip(rgbas[half_idx:, :3] + sheen_factor, 0.0, 1.0)
        return rgbas
    ...
    def get_subpaths_from_points(self, points: CubicBezierPath) -> list[CubicSpline]:
        ...
        OPTIMIZED: Completely vectorizes 3D continuity checks across all curve 
        boundaries, bypassing scalar Python filter loops and lambda overhead.
        """
        nppcc = self.n_points_per_cubic_curve
        if len(points) <= nppcc:
            return [points] if len(points) == nppcc else []
            
        # Extract previous curve end anchors and next curve start anchors as blocks
        p0 = points[nppcc - 1 : -1 : nppcc]
        p1 = points[nppcc : : nppcc]
        atol = self.tolerance_for_point_equality
        rtol = 1e-05
        
        # Vectorized multi-axis comparison (corresponds to 'not allclose')
        split_mask = np.any(np.abs(p0 - p1) > (atol + rtol * np.abs(p1)), axis=1)
        
        boundary_indices = np.arange(nppcc, len(points), nppcc)
        filtered_indices = boundary_indices[split_mask]
        
        split_indices = [0] + filtered_indices.tolist() + [len(points)]
        return [
            points[i1:i2]
            for i1, i2 in zip(split_indices[:-1], split_indices[1:], strict=True)
            if (i2 - i1) >= nppcc
    ...
    def gen_subpaths_from_points_2d(self, points: CubicBezierPath) -> Iterable[CubicSpline]:
        """Given an array of points defining the bezier curves of the vmobject, 
        return subpaths formed by these points in 2D space.
        
        OPTIMIZED: Eliminates over 1.6 million recursive Python calls by matching 
        all X and Y coordinate boundaries in parallel within C-compiled memory.
        """
        nppcc = self.n_points_per_cubic_curve
        if len(points) <= nppcc:
            return (points,) if len(points) == nppcc else ()
            
        # Continuous C-contiguous slicing for coordinate boundaries
        p0 = points[nppcc - 1 : -1 : nppcc]
        p1 = points[nppcc : : nppcc]
        atol = self.tolerance_for_point_equality
        rtol = 1.0e-5
        
        # Vectorized check for 2D point inequalities (X and Y coordinates only)
        diff_x = np.abs(p0[:, 0] - p1[:, 0])
        diff_y = np.abs(p0[:, 1] - p1[:, 1])
        split_mask = (diff_x > (atol + rtol * np.abs(p1[:, 0]))) | (diff_y > (atol + rtol * np.abs(p1[:, 1])))
